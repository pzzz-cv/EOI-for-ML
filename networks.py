"""
networks.py — 光流修复网络结构定义（备用，未集成到当前推理流程）

包含以下网络：
  - BaseNetwork       : 所有网络的基类，提供权重初始化方法
  - InpaintGenerator  : 图像级 inpainting 生成器（encoder-residual-decoder，输入 3 通道 RGB）
...
class InpaintGenerator(BaseNetwork):
    """
    图像级 Inpainting 生成器（encoder-residual-decoder 架构）。

    结构：
      encoder : 3 → 64 → 128 → 256，步长卷积下采样 2 次
      middle  : 8 个 ResnetBlock（膨胀率 2），特征维持 256 通道
      decoder : 256 → 128 → 64 → 2，反卷积上采样 2 次，输出 2 通道光流
    输入：3 通道 RGB 图像
    输出：2 通道光流预测（经 tanh 归一化到 [0, 1]）
    """
